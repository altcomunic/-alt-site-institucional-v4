...
function Reveal({ children, delay = 0, className = "" }) {
  return (
    <motion.div className={className} initial={{ opacity: 0, y: 32, filter: "blur(8px)" }} whileInView={{ opacity: 1, y: 0, filter: "blur(0px)" }} viewport={{ once: true, amount: 0.22 }} transition={{ duration: 0.7, delay, ease: [0.22, 1, 0.36, 1] }}>
      {children}
    </motion.div>
  );
}
...
function SectionTitle({ eyebrow, title, text }) {
  return <Reveal className="section-copy"><p className="eyebrow">{eyebrow}</p><h2>{title}</h2>{text ? <p className="section-lead">{text}</p> : null}</Reveal>;
}
...
function Hero() {
  return (
    <section id="home" className="hero section-full"><video className="hero-video" autoPlay muted loop playsInline><source src={heroVideo} type="video/mp4" /></video><div className="hero-overlay" /><div className="container hero-grid"><Reveal><p className="eyebrow muted">Marketing · Branding · Performance</p><h1>Marketing sem achismo.<span>Resultado que converte.</span></h1><p className="hero-lead">Estratégia, criatividade e direção comercial para empresas que querem crescer com consistência.</p><div className="hero-actions"><a className="btn primary" href="#diagnostico" data-cursor="Contato">Solicitar diagnóstico <ArrowRight size={16} /></a><a className="btn ghost" href="#cases" data-cursor="Cases">Conhecer cases</a></div></Reveal><Reveal delay={0.15} className="hero-topics"><a>Branding</a><a>Performance</a><a>CRM</a><a>Dados</a><a>Growth</a></Reveal></div></section>
  );
}

function Manifesto() {
  return <section className="manifesto section-block"><div className="container manifesto-flow"><p className="eyebrow">Manifesto ALT</p>{["Marketing sem direção vira produção.", "Marketing com direção vira crescimento.", "Nós não vendemos posts.", "Construímos sistemas de aquisição."].map((text, index) => <Reveal key={text} delay={index * 0.04}><h2>{text}</h2></Reveal>)}</div></section>;
}

function Modal({ title, children, onClose }) {
  return <motion.div className="modal" initial={{ opacity: 0 }} animate={{ opacity: 1 }}><button className="modal-bg" onClick={onClose} aria-label="Fechar" /><motion.div className="modal-panel" initial={{ x: "100%" }} animate={{ x: 0 }} transition={{ duration: 0.45, ease: [0.22, 1, 0.36, 1] }}><button className="modal-close" onClick={onClose} aria-label="Fechar"><X size={22} /></button><p className="eyebrow">Ecossistema ALT</p><h2>{title}</h2>{children}</motion.div></motion.div>;
}
...
function Cinematic() {
  return <section className="cinema section-block"><video autoPlay muted loop playsInline><source src={cinemaVideo} type="video/mp4" /></video><div className="container cinema-copy"><Reveal><p className="eyebrow">04 — Resultado</p><h2>Enquanto muitos medem curtidas,<br />nós medimos crescimento.</h2><div className="dash"><span>CRM</span><span>Marca</span><span>Performance</span><span>Receita</span></div></Reveal></div></section>;
}

function Insights() {
  return <section id="insights" className="section-block insights-section"><div className="container"><SectionTitle eyebrow="05 — Leituras ALT" title="Pensamento estratégico para empresas em movimento." /><div className="blog-grid">{posts.map(([cat, time, title, desc], index) => <motion.article key={title} className="blog-card" data-cursor="Ler" initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.05 }}><div><span>{cat}</span><em>{time} de leitura</em></div><h3>{title}</h3><p>{desc}</p><a href="#insights">Ler artigo <ArrowRight size={14} /></a></motion.article>)}</div></div></section>;
}

function Partners() {
  return <section id="parceiros" className="section-block partners-section"><div className="container"><SectionTitle eyebrow="06 — Parceiros" title="Ecossistema tecnológico." text="As plataformas e parceiros que utilizamos para transformar estratégia em operação." /><div className="partner-grid"><a className="partner-card" href="https://www.kommo.com/br/" target="_blank" rel="noreferrer"><img src="/kommo-partner.svg" alt="Kommo Partner" /><p>CRM comercial, automação e pipeline.</p><span>Conhecer parceiro <ArrowRight size={14} /></span></a><a className="partner-card synapse" href="https://www.synapsestrateg.com" target="_blank" rel="noreferrer"><strong>Synapse</strong><p>IA aplicada, automações e treinamentos.</p><span>Conhecer parceiro <ArrowRight size={14} /></span></a></div></div></section>;
}
...
export default function App() {
  ...
  return <div><motion.div className="scroll-progress" style={{ width }} /><SmartCursor />{loading ? <div className="loader"><Logo /></div> : null}<Navbar /><main><Hero /><Manifesto /><System /><Method /><Cases /><Cinematic /><Insights /><Partners /><Contact /></main><Footer /></div>;
}
